...
- [ ] A host account ready (Render is easiest with `render.yaml`)
- [ ] Domain available to point (e.g. app.nichefinderhq.com)

## Saturday sequence

**1. Deploy (15 min).** In Render: New → Blueprint → point at this repo. It reads
`render.yaml` and provisions the service + persistent disk automatically.

**2. Paste secrets (5 min)** in the Render dashboard (never in the repo):
- `ANTHROPIC_API_KEY` = your key
- `STRIPE_SECRET_KEY` = `sk_test_…` (test first!)
- `STRIPE_WEBHOOK_SECRET` = from step 4
- `PUBLIC_ORIGIN` = your final https URL
- `WALLET_STORE_KEY`, `ADMIN_API_KEY` = auto-generated by the blueprint — leave them
...
- **`/v1/health` won't load** → deploy failed; check Render logs for a missing env var.
...
- **Anything worse** → the encrypted wallet store and Stripe are the source of truth; a redeploy never loses money or balances. Roll back to the previous Render deploy in one click.
